    ...
    <div className="app-shell section-block max-w-4xl">
      ...
      <SoftCard className="p-8 md:p-10">
        {shareState.profile?.nickname ? (
          <div className="mb-8 flex items-center gap-4 rounded-[24px] bg-gray-50 p-5">
            ...
            <div>
              ...
              <p className="mt-1 text-xl font-bold text-gray-900">{shareState.profile.nickname}</p>
            </div>
          </div>
        ) : null}
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          {shareState.interests.length > 0 ? (
            shareState.interests.map((interest) => (
              <div key={interest} className="rounded-[24px] border border-gray-200 bg-gray-50 p-5">
                <p className="text-xl font-extrabold text-gray-900">{interest}</p>
                <p className="mt-2 text-base text-gray-600">{shareState.subInterests[interest] ?? "세부 관심 선택 안 함"}</p>
              </div>
            ))
          ) : (
            <p className="text-base text-gray-600">공유된 주제가 없습니다. 기본 가입 화면으로 이동해 주세요.</p>
          )}
        </div>
