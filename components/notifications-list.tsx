      ...
      {notifications.length > 0 ? (
        <div className="flex justify-end">
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center rounded-2xl border border-navy-200 bg-white px-4 py-2 text-sm font-semibold text-navy-800 transition hover:border-navy-300 hover:bg-navy-50"
          >
            전체 읽음 처리
          </button>
        </div>
      ...
      {notifications.map((item) => (
        <button
          ...
          className={`w-full rounded-xl border p-5 text-left transition ${
            item.is_read ? "border-navy-100 bg-white" : "border-orange-200 bg-orange-50"
          }`}
        >
          ...
          <div className="mt-3">
            {extractActorName(item.actor_name, item.title) ? (
              <p className="text-base font-bold leading-snug text-navy-900">{extractActorName(item.actor_name, item.title)}</p>
            ) : null}
            <p className="mt-1 text-sm leading-7 text-navy-600">{item.body}</p>
          </div>

          {/* 행 3: 일시 */}
          <div className="mt-2 text-right">
            <p className="text-xs text-navy-400">{new Date(item.created_at).toLocaleString("ko-KR")}</p>
          </div>
        </button>
      ))}
      {notifications.length === 0 ? (
        <div className="rounded-xl border border-dashed border-navy-200 bg-white p-6 text-sm text-navy-500">아직 알림이 없습니다.</div>
      ) : null}
    </div>
  );
}
