...
const { data: users } = await supabase.from('sj_users').select("id, email").limit(1);
...
await supabase.from('sj_user_interest_selections').delete().eq("user_id", userId).eq("main_interest", "취미");
console.log("\n[cleanup done]");
