...
//
// QA 발견 (2026-05-19):
//   - 친구 글 → "피크닉 용품" → 캠핑매트 17,800원 추천 (친구 만남에 매트?)
//   - 의료비/보험 글 → "베스트셀러 도서" → 니체/Atomic Habits 추천 (전혀 무관)
//   - 재테크 글 → 같은 자기계발 책 (가계부 X)
// → 매핑 룰을 글 주제와 즉시 행동 (선물 / 사용) 가능한 상품으로 다듬음.
const CATEGORY_KEYWORD_MAP: Record<string, string[]> = {
  // 메인 카테고리 폴백 — 카테고리 안의 어떤 글이 와도 부적절하지 않을 정도로 범용 + 즉시 효용.
  실생활: ["무선청소기", "에어프라이어", "정리수납함", "생활용품"],
  건강: ["종합영양제", "혈압계", "마사지건", "체중계"],
  돈: ["가계부", "재테크도서", "다이어리"],
  뉴스: ["텀블러", "무선이어폰", "베스트셀러도서"],
  관계: ["선물세트", "꽃다발", "케이크 기프티콘", "감성다이어리"],
  // ── 서브 — 글 sub_interest 가 명시되면 우선 사용 ──
  // 건강 계열
  혈압: ["혈압계", "혈압 관리"],
  관절: ["관절 보호대", "무릎 보호대"],
  음식: ["건강식품", "견과류세트"],
  ...
  연금: ["은퇴재무 도서", "가계부"],
  세금: ["세금 도서", "가계부"],
  보험: ["혈압계", "건강기능식품"],   // 보험 글은 결국 건강관리 — 매칭 더 자연스러움
  주의: ["보안카드케이스", "신용카드 케이스"],
  혜택: ["가계부", "재테크도서"],
  // 실생활 계열
  ...
  해외: ["여행 어댑터", "여행용 캐리어"],
  // 관계 계열 — 친구/부부/가족/회사
  가족: ["가족 선물세트", "효도 선물", "건강기능식품"],
  부부: ["커플 선물", "기념일 선물", "와인", "꽃다발"],
  회사: ["선후배 선물세트", "다과 세트", "사무용품"],
  취미: ["취미용품"],
  친구: ["친구 선물", "케이크 기프티콘", "다과 세트", "감성다이어리"], // 캠핑매트 매핑 제거
  // 인간관계 일반 — 글 sub 가 없을 때 추가 폴백으로
  마음: ["감성다이어리", "캘리그라피 책", "위로 에세이"],
};

/**
 * 새 매핑 키워드는 cache 가 비어있을 수 있음 (운영 DB 의 sj_coupang_product_cache).
 * scripts/build-coupang-cache.mjs 같은 cron 으로 사전 캐시 빌드를 권장.
 * 임시로 cache miss 시 더 일반적인 fallback 으로 떨어지는 마지막 키워드:
 *   "베스트셀러도서" — 기존에 캐시되어 있는 거의 모든 경우 hit.
 * 단, 글과 무관한 추천을 막기 위해 카테고리 매핑에 직접 넣지 않고
 * fetchPopularProductsForContent 가 모든 키워드 시도 후 0개일 때만 사용.
 */
const ABSOLUTE_FALLBACK_KEYWORDS = ["베스트셀러도서", "텀블러"];
...
export async function fetchPopularProductsForContent(
  ...
  // 3단계: 절대 폴백 — 위 두 단계 모두 cache miss 면 빈 카드. UI 가 빈 자리.
  // 운영 마이그레이션 중 새 키워드 캐시 빌드 전까지의 안전망.
  if (results.length === 0) {
    for (const keyword of ABSOLUTE_FALLBACK_KEYWORDS) {
      if (results.length >= limit) break;
      try {
        const items = await requestTopSearchProducts(keyword, limit * 3, 0, effectiveCacheOnly);
        addItems(items, keyword);
